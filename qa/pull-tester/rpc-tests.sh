...
export CUCKOOCOINCLI=${BUILDDIR}/qa/pull-tester/run-cuckoocoin-cli
export CUCKOOCOIND=${REAL_CUCKOOCOIND}
...
if [ "x${ENABLE_CUCKOOCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and cuckoocoind must all be enabled"
fi
