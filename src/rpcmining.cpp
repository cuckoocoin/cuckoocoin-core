...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or cuckoocoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateCuckoocoins(fGenerate, pwalletMain, nGenProcLimit);
...
// NOTE: Unlike wallet RPC (which use CUC values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.cuckoocoin.it/wiki/BIP_0022 for full specification.\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Cuckoocoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Cuckoocoin is downloading blocks...");
